...
- **Фавикон** лендинга: `site/favicon.svg`+`favicon.ico`+`apple-touch-icon.png` (бренд-иконки ratescout) + `<link>` в index/404.
- **Секция «Консольный монитор»** на лендинге: команды запуска (`irm .../cli/rs.ps1 | iex`, `bash <(curl -s .../cli/rs.sh)`), легенда клавиш, ссылки на исходники rs.ps1/rs.sh. Сам фид генерит основной репо ratescout (`make_cli_pages`), живёт на ratescout.ru/cli/.
- **FAQ** на лендинге (`site/index.html`): 9 вопросов (аккордеон `<details>`) + JSON-LD **FAQPage** (schema.org) для rich-результатов. При правке — число вопросов в схеме держать равным числу `<summary>`.
- **Установка на iPhone/iPad через профиль Web Clip** (`site/ratescout.mobileconfig`, тип `com.apple.webClip.managed`): иконка RateScout на экран «Домой» → открывает `ratescout.ru/monitor/` во весь экран (FullScreen, UTM `utm_source=ios_webclip`). Иконка 180×180 вшита base64. Профиль генерить через `plistlib.dump` (сам экранирует `&` в URL; вручную легко словить `not well-formed`). На лендинге — карточка iPhone/iPad + инструкция (открывать в **Safari**, профиль неподписан → «Не проверено», прав не просит). 🔴 Нативного App Store приложения НЕТ (нужен платный Apple Developer — решение ПМ). ⚠️ На проверку человеком: реальный iPhone (Safari→установка профиля→иконка); отдаёт ли Pages `.mobileconfig` так, что Safari предлагает установку (возможен octet-stream).
